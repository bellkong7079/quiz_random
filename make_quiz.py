...
.q-text{font-size:.95rem;line-height:1.65}
pre.code-block{
  background:#1e1e2e;color:#cdd6f4;
  padding:13px 16px;border-radius:8px;
  font-size:.84rem;font-family:Consolas,"Courier New",monospace;
  line-height:1.6;margin:10px 0 13px;
  overflow-x:auto;white-space:pre}
pre.code-block.sql{background:#1a2a1a;color:#a8d8a8}
pre.code-block.python{background:#1a1a2e;color:#cdd6f4}
...
.source-tag{font-size:.7rem;color:#bbb;margin-top:5px}
#new-btn{display:block;margin:36px auto 0;background:#1a73e8;color:#fff;border:none;
...
.modal-box{background:#fff;border-radius:16px;
  padding:36px 44px;text-align:center;max-width:340px;width:90%;
...
.m-btn{background:#1a73e8;color:#fff;border:none;border-radius:8px;padding:11px 28px;
...
function render(qs){
  ...
  qs.forEach((q,i)=>{
    ...
    const codeHtml = q.code
      ? `<pre class="code-block ${q.lang||""}">${esc(q.code)}</pre>`
      : "";
    ...
    d.innerHTML=
      ...
      `</div>` +
      codeHtml +
      `<div class="choices" id="ch-${i}"></div>` +
      `<div class="result-msg" id="msg-${i}"></div>` +
      `<div class="source-tag">[출처: ${esc(q.source)}]</div>`;
  ...
  C.insertAdjacentHTML("beforeend",
    `<button id="new-btn" onclick="startNew()">새 문제 60개 뽑기</button>`);
}
...
function startNew(){
  ...
  document.getElementById("round-info").textContent=
    "전체 "+ALL_QS.length+"문제 중 랜덤 "+Math.min(60,ALL_QS.length)+"문제 ("+roundN+"회차)";
